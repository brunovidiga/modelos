...
class HeroEffect {
    ...
    async init() {
        ...
        const tex1 = await loadTexture('/woman.jpg');
        const tex2 = await loadTexture('/panther.jpg');
        const depthTex1 = await loadTexture('/woman_depth.jpg');
        const depthTex2 = await loadTexture('/panther_depth.jpg');

        // Calcular proporção baseada na primeira imagem
        const img = tex1.image;
        this.imgAspect = img.width / img.height;
        
        // Criar geometria 1x1 para escalonar via resize e cobrir a tela (cover)
